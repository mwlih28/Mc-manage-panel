...
- **Community Egg Store** — Admin → Eggs → Browse Community Eggs pulls in 299 real, ready-to-use game/service eggs (Minecraft, SteamCMD titles, voice servers, databases, Discord bots, and more) with one-click single or bulk import, alongside a manual import/export for any custom Pterodactyl-format egg JSON. Fresh installs now seed the full catalog automatically so a new panel doesn't start with an empty Eggs page.
- **hCaptcha** — an optional CAPTCHA challenge on the Login and Register forms, configurable from Admin → Settings. Off by default; every existing install is unaffected until an admin adds their own hCaptcha site/secret key.
- **Real SFTP access** — Wings now runs an actual SFTP server (previously just a placeholder config port), scoped strictly to a server's own data directory. Login with `<panel-username>.<server-short-id>` and the same password used to sign into the panel — matching Pterodactyl's convention for anyone coming from it.
