...
import {
  Search, Hammer, Download, Trash2, RefreshCw, ArrowUpCircle,
  Power, PowerOff, ExternalLink, X, Users, ArrowDownToLine, Info,
} from 'lucide-react';
...
const SUPPORTED_LOADERS = ['fabric']; // the only one with an egg/install script today
...
const MANIFEST_PATH = '/mods/.kretase-manifest.json';
...
function pickBestVersion(versions: ModrinthVersion[], loader: string, mcVersion?: string): { version: ModrinthVersion; exact: boolean } {
  let candidates = versions.filter((v) => v.loaders?.some((l) => l.toLowerCase() === loader));
  if (candidates.length === 0) candidates = versions;
  let exact = true;
  if (mcVersion) {
    const exactMatch = candidates.filter((v) => v.game_versions?.includes(mcVersion));
    if (exactMatch.length > 0) {
      candidates = exactMatch;
    } else {
      exact = false;
      const majorMinor = mcVersion.split('.').slice(0, 2).join('.');
      const minorMatch = candidates.filter((v) =>
        v.game_versions?.some((gv) => gv === majorMinor || gv.startsWith(`${majorMinor}.`))
      );
      if (minorMatch.length > 0) candidates = minorMatch;
    }
  }
  return { version: candidates[0], exact };
}

export function ModManager({ serverId, mcVersion, eggName }: { serverId: string; mcVersion?: string; eggName?: string }) {
  const detectedLoader = LOADERS.find((l) => eggName?.toLowerCase().includes(l)) ?? 'fabric';
  ...
  const isSupportedLoader = SUPPORTED_LOADERS.includes(loader);

  const runSearch = useCallback(async (offset: number, append: boolean) => {
    if (append) setLoadingMore(true); else setSearching(true);
    try {
      const facetGroups: string[][] = [['project_type:mod'], [`categories:${loader}`]];
  ...
  }, [debouncedQuery, categories, sort, loader]);
  ...
  const loadInstalled = useCallback(async () => {
    ...
    try {
      const [{ data }, m] = await Promise.all([
        api.get(`/servers/${serverId}/files`, { params: { directory: '/mods' } }),
        loadManifest(),
      ]);
  ...
  const install = async (project: ModrinthProject) => {
    ...
    try {
      ...
      if (!allVersions.length) { toast.error('No versions found for this mod'); return; }

      const { version: best, exact } = pickBestVersion(allVersions, loader, mcVersion);
      if (!best) { toast.error(`No ${loader} build found`); return; }
      if (!mcVersion) toast('Tip: install a version first for accurate matching.', { icon: 'ℹ️' });
      ...
      await api.post(`/servers/${serverId}/plugins/install`, { url: file.url, filename: file.filename, type: 'mods' });
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.post(`/servers/${serverId}/files/delete`, { files: [`/mods/${deleteTarget.name}`] });
      const m = await loadManifest();
  ...
  const toggleEnabled = async (file: FileEntry) => {
    ...
    try {
      await api.put(`/servers/${serverId}/files/rename`, { from: `/mods/${file.name}`, to: `/mods/${newName}` });
      const m = await loadManifest();
      if (m[file.name]) {
      ...
      toast.success(isDisabled ? 'Mod enabled' : 'Mod disabled');
      loadInstalled();
    } catch {
    ...
    try {
      ...
      for (const [filename, meta] of entries) {
        try {
          ...
          const { version: latest } = pickBestVersion(versions, loader, mcVersion);
          if (latest && latest.id !== meta.versionId) found[filename] = latest;
        } catch { /* skip this mod, keep checking the rest */ }
      }
      setUpdates(found);
      ...
      if (unmanaged.length > 0) {
        try {
          const { data } = await api.get(`/servers/${serverId}/files/detect`, { params: { directory: '/mods' } });
          ...
          for (const row of rows) {
            if (row.modrinth) {
              ...
              try {
                ...
                const versions: ModrinthVersion[] = await res.json();
                latestVersion = pickBestVersion(versions, loader, mcVersion).version;
      ...
      if (total === 0) toast.success('All mods are up to date');
  ...
  const applyDetectedUpdate = async (entry: DetectedEntry) => {
    ...
    try {
      if (entry.source === 'modrinth' && entry.modrinth?.latestVersion) {
        ...
        if (!file) return;
        await api.post(`/servers/${serverId}/files/delete`, { files: [`/mods/${entry.filename}`] });
        await api.post(`/servers/${serverId}/plugins/install`, { url: file.url, filename: file.filename, type: 'mods' });
        ...
        await api.post(`/servers/${serverId}/files/delete`, { files: [`/mods/${entry.filename}`] });
        await api.post(`/servers/${serverId}/plugins/install`, { url: target.downloadUrl, filename: target.fileName, type: 'mods' });
      }
  ...
  const applyUpdate = async (filename: string) => {
    ...
    try {
      ...
      if (!file) return;
      await api.post(`/servers/${serverId}/files/delete`, { files: [`/mods/${filename}`] });
      await api.post(`/servers/${serverId}/plugins/install`, { url: file.url, filename: file.filename, type: 'mods' });
    ...
    <div className="space-y-4">
      {!isSupportedLoader && (
        <div className="flex items-start gap-2 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 text-xs">
          ...
          <span>
            This server's egg doesn't look like a supported mod loader yet — only <strong>Fabric</strong> has an
            install script today. Mods installed here won't load unless the server is running Fabric.
          </span>
        </div>
      )}
      ...
      <div className="card">
        <div className="card-header flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-slate-100">Installed Mods</h3>
            <p className="text-xs text-slate-500 mt-0.5">{installed.length} mod(s) in /mods</p>
          </div>
        ...
        <div className="divide-y divide-dark-800/50">
          {installedLoading ? (
            ...
            <div className="p-8 text-center text-slate-500">
              <Hammer size={28} className="mx-auto mb-2 opacity-20" />
              <p className="text-sm">No mods installed</p>
            </div>
          ) : (
            installed.map((mod) => {
              const isDisabled = mod.name.endsWith('.disabled');
              const meta = manifest[mod.name];
              const update = updates[mod.name];
              return (
                <div key={mod.name} className="flex items-center gap-3 px-4 py-3">
                  {meta?.iconUrl ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className={cn('text-sm font-medium truncate', isDisabled ? 'text-slate-500' : 'text-slate-200')}>
                        {meta?.title ?? mod.name.replace(/\.disabled$/, '')}
                      </span>
                    ...
                    <p className="text-xs text-slate-600 font-mono truncate">{mod.name}</p>
                  </div>
                  <span className="text-xs text-slate-600 shrink-0">{formatBytes(mod.size)}</span>
                  {update && (
                    <button
                      className="p-1.5 text-blue-400 hover:text-blue-300 hover:bg-blue-500/10 rounded transition-colors shrink-0"
                      onClick={() => applyUpdate(mod.name)}
                      disabled={installing === mod.name}
                      title={`Update to ${update.version_number}`}
                    >
                      {installing === mod.name ? <Spinner size="sm" /> : <ArrowDownToLine size={14} />}
                    </button>
                  )}
                  <button
                  ...
                  >
                    {toggling === mod.name ? <Spinner size="sm" /> : isDisabled ? <Power size={14} /> : <PowerOff size={14} />}
                  </button>
                  <button
      ...
      {/* Hash-detected mods that aren't tracked in the manifest yet
      ...
      <div className="card">
        <div className="card-header">
          <h3 className="text-sm font-semibold text-slate-100">Mod Marketplace</h3>
          <p className="text-xs text-slate-500 mt-0.5">
            Search and install mods directly to your server
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search mods (e.g. Sodium, Jade, Create)..."
                value={query}
                onChange={(e) => setQuery(e.target.value)}
              />
            </div>
            <select className="input w-36" value={loader} onChange={(e) => setLoader(e.target.value)}>
              {LOADERS.map((l) => (
                <option key={l} value={l}>{l[0].toUpperCase() + l.slice(1)}{!SUPPORTED_LOADERS.includes(l) ? ' (soon)' : ''}</option>
              ))}
            </select>
