...
async function getPanelSmtpConf() {
    const rows = await prisma_1.prisma.setting.findMany({ where: { key: { startsWith: 'smtp.' } } });
    const conf = {};
    for (const r of rows)
        conf[r.key] = r.value;
    return conf;
...
async function getSmtpConf() {
    return getRegistrySmtp() ?? getPanelSmtpConf();
}
function makeTransport(conf) {
    if (!conf['smtp.host'] || !conf['smtp.user'] || !conf['smtp.pass'])
        return null;
...
function thankYouHtml(name, serverIp) {
...
function updateHtml(version, changelogUrl) {
    return `<!DOCTYPE html>
...
async function sendThankYouEmail(to, name, serverIp) {
    try {
        ...
        logger_1.logger.info(`Thank-you email sent to ${to}`);
        return true;
    }
    catch (err) {
        logger_1.logger.warn(`Failed to send thank-you email to ${to}: ${err.message}`);
        return false;
    }
}
async function sendOwnerNotification(name, email, serverIp, domain) {
    try {
...
<tr><td style="padding:6px 0;color:#71717a;">Time</td><td style="color:#e4e4e7;">${new Date().toUTCString()}</td></tr>
        ...
        logger_1.logger.info(`Owner notification sent for installation from ${serverIp}`);
        return true;
    }
    catch (err) {
        logger_1.logger.warn(`Failed to send owner notification: ${err.message}`);
        return false;
    }
}
function resetPasswordHtml(resetUrl, appName) {
    return `<!DOCTYPE html>
...
async function sendPasswordResetEmail(to, resetUrl, appName = 'Kretase') {
    try {
        const conf = await getPanelSmtpConf();
        const transport = makeTransport(conf);
        if (!transport) {
            logger_1.logger.warn('Password reset email not sent — panel SMTP is not configured');
            return false;
        }
        const from = conf['smtp.from'] || conf['smtp.user'];
        await transport.sendMail({
        ...
        logger_1.logger.info(`Password reset email sent to ${to}`);
        return true;
    }
    catch (err) {
        logger_1.logger.warn(`Failed to send password reset email to ${to}: ${err.message}`);
        return false;
    }
}
async function sendUpdateNotification(to, version, changelogUrl) {
    try {
        const conf = await getSmtpConf();
        const transport = makeTransport(conf);
        if (!transport)
        ...
        logger_1.logger.warn(`Failed to send update notification to ${to}: ${err.message}`);
        return false;
    }
}
